...
export const selectNextMenu = createSelector(selectMenuState, fromMenu.getNextMenus);
...
export const is_data_ready = createSelector(
  ...
  selectMenuEntities,
  (menus, menusEntities) => {
    const dataMenus = menus.map((id) => menusEntities[id].data_id).filter((menu) => !!menu);
    const periodMenus = menus.map((id) => menusEntities[id].type).filter((menu) => menu === 'period');
...
export const selectedDatas = createSelector(
  selectNextMenu,
  selectMenuEntities,
  (menus, menusEntities) => menus.map((id) => menusEntities[id].data_id).filter((menu) => menu !== '')
);
